...
- **What existed vs. from zero (confirmed 2026-09-23):** Cogsworth already had scheduling (with many problems; clients constantly used workarounds to get where they wanted), custom forms (with problems), patient lists, a practitioner list (he believes so, 2026-09-23) and settings (some very complex). Fernando redesigned the UX and UI of these; everything else was designed from zero.
- **More scope (confirmed 2026-09-23):** everything was designed responsive for mobile. The waiting room was designed as an experience of its own. Onboarding strategy (the product had none). Billing, invoicing and coupons. Private notes vs. progress notes. Customisable dashboards. Five personas: practitioner, front desk, owner/admin, manager, and the client via the patient portal. Smaller-but-big features inside each area, e.g. custom fields, multi-question form builder, Face Vitals inside records, availability time slots for scheduling, "and many many more". Onboarding (confirmed): a strategy of the fewest possible steps to configure as much of the platform as possible with minimal friction; the practice customises its app from its choices (brand colour applied across the app); guided setup with gamification, skippable at any moment. Progress notes vs. private notes are the standard technical terms in psychology practice. Coupons are a minor feature inside billing and invoicing: mention without detail. "Face Vitals" was the first name of Vitals AI; mentioning it is optional, the site uses Vitals AI.
...
- **Features (confirmed 2026-09-23):** patient records open on a per-patient dashboard with tabs (appointments, notes, documents, forms, personal information). Telehealth was designed end to end by Fernando. Messaging is free-form (not per patient or appointment). Check-in and billing were designed and count as a product area (9 areas now, not 8). Patient portal: shared information, upcoming appointments, booking. He expects further whole areas are missing from the case and will confirm them.
- **Clinical notes (confirmed 2026-09-23):** designed in the design files: note tied to the appointment, start/end recorded, writing window after the consultation, per-field privacy, templates per specialty, block editor, dictation, permissions by role/team, timeline with pins, PDF export. SimplePractice was studied in depth and is named in both the Platform and AI benchmarks.
- **Notion export (`~/Downloads/…ExportBlock…/Upvio`):** detailed product definition for Notes and Dashboards, design-issue categorisation and the QA backlog CSV (~40 issues), plus product screenshots from April 2024 in the QA pages. The Perplexity `.md` files in Downloads match `docs/` and only detail scheduling.
...
- **Real navigation (confirmed 2026-09-23, from a product screenshot):** sidebar with Home, Calendar, Clients, Video & Chat, Forms, Staff, Services, Locations; Settings at the bottom; the active practice (name and staff count) below it. The product calls patients "Clients", but the site says "patients" everywhere, including the diagram (decided 2026-09-23). The `ia-before-after` diagram now shows this nav; what lives under each entry is confirmed as mostly right: Vitals AI lives in Clients; staff availability is set in the Calendar; the waiting room sits under Video & Chat in simple terms (he will explain the nuance later).
