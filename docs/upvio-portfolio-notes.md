...
- **Real navigation (confirmed 2026-09-23, from a product screenshot):** sidebar with Home, Calendar, Clients, Video & Chat, Forms, Staff, Services, Locations; Settings at the bottom; the active practice (name and staff count) below it. The product calls patients "Clients". The `ia-before-after` diagram now shows this nav; what lives under each entry (e.g. check-in in Calendar, Vitals AI in Clients, waiting room in Video & Chat) is inferred and awaits confirmation.
