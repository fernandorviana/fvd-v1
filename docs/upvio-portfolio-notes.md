...
- **Real navigation (confirmed 2026-09-23, from a product screenshot):** sidebar with Home, Calendar, Clients, Video & Chat, Forms, Staff, Services, Locations; Settings at the bottom; the active practice (name and staff count) below it. The product calls patients "Clients". The `ia-before-after` diagram now shows this nav; what lives under each entry is confirmed as mostly right: Vitals AI lives in Clients; staff availability is set in the Calendar; the waiting room sits under Video & Chat in simple terms (he will explain the nuance later).
